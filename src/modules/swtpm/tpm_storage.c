...
#include <swtpm/tpm_handles.h>
#include <swtpm/tpm_marshalling.h>
                             ...
                             TPM_KEY *wrappedKey)
{
  ...
  printk("[wizrampa] get parent key\n");
  /* verify authorization */
  res = tpm_verify_auth(auth1, parent->usageAuth, parentHandle);
  if (res != TPM_SUCCESS) return res;
  auth1->continueAuthSession = FALSE;
  printk("[wizrampa] verify authorization\n");
  session = tpm_get_auth(auth1->authHandle);
  printk("[wizrampa] get auth handle\n");
  ...
  if ((keyInfo->keyUsage == TPM_KEY_STORAGE
    ...
    return TPM_BAD_KEY_PROPERTY;
  printk("[wizrampa] verify key parameter\n");
  /* setup the wrapped key */
  memcpy(wrappedKey, keyInfo, sizeof(TPM_KEY));
  printk("[wizrampa] setup the wrapped key\n");
  /* setup key store */
  ...
  printk("[wizrampa] setup key store\n");
  ...
  printk("[wizrampa] rsa key generate\n");
  ...
  tpm_rsa_release_private_key(&rsa);
  printk("[wizrampa] rsa key stores\n");
  ...
  printk("[wizrampa] compute key digest\n");
  ...
  printk("[wizrampa] encrypt private key data\n");
  tpm_free(store.privKey.key);
  return TPM_SUCCESS;
}
