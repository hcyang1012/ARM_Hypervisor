...
#include <swtpm/tpm_structures.h>
...
{
  #ifdef CONFIG_XEN
  //Xen version doesn't support this function.
  #endif
  return 0;
}
...
void tpm_init_data(void)
{
  ...
  /* set TPM vision */
  memcpy(&tpmData.permanent.data.version, &tpm_version, sizeof(TPM_VERSION));
    /* seed PRNG */
      ...
      if (tpmConf & TPM_CONF_GENERATE_EK) {
        ...
        if (tpmConf & TPM_CONF_GENERATE_SEED_DAA) {
          ...
          int tpm_store_permanent_data(void)
          {
            ...
            //This function is not used in xen version.
            #else
            uint8_t *buf, *ptr;
            size_t buf_length;
            uint32_t len;
