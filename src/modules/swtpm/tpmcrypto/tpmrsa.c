...
#include <swtpm/tpmcrypto/tpmsha1.h>
#include <swtpm/tpmcrypto/tpmrsa.h>
#include <swtpm/tpm_commands.h>
...
static void rsa_tpm_bn_random(tpm_bn_t a, size_t nbits)
{
  ...
  printk("[wizrampa] get random bytes\n");
  tpm_bn_import(a, size, 1, buf);
  printk("[wizrampa] import buffer\n");
}
...
int tpm_rsa_generate_key(tpm_rsa_private_key_t *key, uint16_t key_size)
{
  ...
  printk("[wizrampa] set key size completion\n");
  do {
    /* get prime p */
    rsa_tpm_bn_random(p, key_size / 2);
  printk("[wizrampa] tpm_bn_random - prime p\n");
    tpm_bn_setbit(p, 0);
  printk("[wizrampa] setbit p 0 - prime p\n");
    tpm_bn_setbit(p, key_size / 2 - 1);
  printk("[wizrampa] setbit key_size / 2 - 1 - prime p\n");
    tpm_bn_setbit(p, key_size / 2 - 2);
  printk("[wizrampa] setbit key_size / 2 - 2 - prime p\n");
    tpm_bn_nextprime(p, p);
  printk("[wizrampa] nextprime - prime p\n");
    tpm_bn_sub_ui(t1, p, 1);
  printk("[wizrampa] sub_ui - prime p\n");
    tpm_bn_gcd(phi, e, t1);
  printk("[wizrampa] tpm_bn_gcd - prime p\n");
    if (tpm_bn_cmp_ui(phi, 1) != 0) continue;
  printk("[wizrampa] get prime p\n");
    /* get prime q */
    rsa_tpm_bn_random(q, key_size / 2);
    tpm_bn_setbit(q, 0);
    tpm_bn_setbit(q, key_size / 2 - 1);
    tpm_bn_setbit(q, key_size / 2 - 2);
  printk("[wizrampa] setbit key_size / 2 - 2 - prime q\n");
    tpm_bn_nextprime(q, q);
  printk("[wizrampa] nextprime - prime q\n");
    tpm_bn_sub_ui(t2, q, 1);
    tpm_bn_gcd(phi, e, t1);
  printk("[wizrampa] tpm_bn_cmp_ui - prime q\n");
    if (tpm_bn_cmp_ui(phi, 1) != 0) continue;
  printk("[wizrampa] get prime q\n");
    /* p shall be smaller than q */
    if (tpm_bn_cmp(p, q) > 0) tpm_bn_swap(p, q);
  printk("[wizrampa] p shall be smaller than q\n");
    /* calculate the modulus */
    tpm_bn_mul(n, p, q);
  printk("[wizrampa] calculate the modulus\n");
  ...
  tpm_bn_clear(u);
  /* test key */
  printk("[wizrampa] start rsa test key\n");
  if (rsa_test_key(key) != 0) {
    tpm_rsa_release_private_key(key);
    return -1;
  }
  printk("[wizrampa] success rsa test key\n");
  return 0;
}
